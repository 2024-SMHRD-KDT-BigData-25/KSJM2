...
<body>
	...
	<form action="SnsWrite" method="post" name="frm"
		...
		<table>
			...
			<!-- 제목 섹션 헤더 -->
			<tr>
				<td><input type="text" placeholder="제목을 입력하세요" name="sns_title"></td>
			</tr>
			<!-- 제목 입력 필드 -->

			<input type="hidden" name="user_id" value="<%=member.getUser_id() %>">
			<!-- 화면에 보이지 않아서 css 연결 안해도 됨 -->
			...
			<!-- 내용 섹션 헤더 -->
			<tr>
				<td><textarea placeholder="내용을 입력하세요" name="sns_content"></textarea></td>
			</tr>
			<!-- 내용 입력 텍스트 영역 -->

			<tr>
				<td><label for="fileUpload" class="file-upload-label">
						<!-- 파일 업로드를 위한 라벨 --> <label> <img src="../img/파일선택.png"
							class="fileChoise"> <input type="file" id="fileUpload"
							name="img" multiple="multiple"> <!-- 숨겨진 파일 입력 필드 -->
					</label>
				</label>
					<div id="fileInfo"></div> <!-- 파일 정보 표시 영역 --></td>
			</tr>
			<tr  align="center">
				<td ><input type="submit" value="작성" onclick="alert('작성 완료!')">
					<!-- 등록 버튼 -->
					<button type="button" class="close-button" onclick="confirm()">취소</button>
					<!-- 닫기 버튼 --></td>
			</tr>
		</table>
	</form>
